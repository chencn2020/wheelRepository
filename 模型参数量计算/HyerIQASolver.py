...
class HyperIQASolver(object):
    ...
    def __init__(self, config, path, train_idx, test_idx):
        ...
        alexnet = self.model_hyper
        alexnet_total_params = sum(p.numel() for p in alexnet.parameters())
        alexnet_total_trainable_params = sum(
            p.numel() for p in alexnet.parameters() if p.requires_grad)
        print("alexnet total parameters. ", alexnet_total_params)
        print("alexnet training parameters. ", alexnet_total_trainable_params)

        # import torchvision
        from torchstat import stat

        # alexnet = torchvision.models.vgg16(pretrained=False)
        res = stat(alexnet, (3, 224, 224))
        print(type(res))
